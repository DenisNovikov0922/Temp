...
class RM_Field_Factory {
    ...
    public function create_default_field() {
        ...
        if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev']) && $this->db_field->field_show_on_user_page) {
            ...
            $this->opts['value'] = empty($user_defaultinfo) ? $this->opts['value'] : $user_defaultinfo;
        }
        return new RM_Frontend_Field_Base($this->db_field->field_id, $this->db_field->field_type, $this->field_name, $this->db_field->field_label, $this->opts, $this->db_field->page_no, $this->db_field->is_field_primary, $this->x_opts);
    }
    ...
    public function create_privacy_field(){
        ...
        $content = str_replace('{{privacy_policy}}',"<a target='_blank' href='$url'>".__('Privacy Policy','registrationmagic-gold')."</a>",$content);
        $icon='';
        $field_label='';
