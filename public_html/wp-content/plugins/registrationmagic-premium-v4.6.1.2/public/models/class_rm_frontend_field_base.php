...
//    }
    ...
    protected function add_custom_validations()
    {
        ...
        if(isset($this->field_model->field_options->field_is_unique))
        {  
            $exclude_value= '';
            if(isset($_GET['submission_id']))
            {
                ...
                if($sub_id>0){
                    $submision_row= RM_DBManager::get("SUBMISSION_FIELDS",array('field_id' => $this->field_model->field_id,'submission_id'=>$sub_id),array('%d','%d'),'results',0,1,'value');
            ...
            $this->pfbc_field->addValidation(new Validation_Unique($this->field_model->field_options->un_err_msg,new RM_Unique_Validator($this->field_model->field_id,$exclude_value)));
        }
        
        if($this->is_primary() && $this->field_type=='Email')
        {  
           $this->pfbc_field->addValidation(new Validation_SubLimitByEmail(RM_UI_Strings::get('ERR_SUB_LIMIT_USER'),new RM_Sub_Limit_Validator($this->field_model->field_id,$this->field_model->get_form_id())));
        }
    }
    ...
    {   
        ...
        if(!empty($this->field_model->field_options->conditions['rules']) && is_array($this->field_model->field_options->conditions['rules'])){
            ...
            if(count($cond_option)>0){
                ...
                $this->field_options['class']= "data-conditional";
            }
