...
class RM_Front_Controller {
    ...
    public function set_otp($model, $service, $request, $params) {
        ...
        if(in_array($sanitized_user_ip, $blocked_ips)){
            echo '{"error":true,"show":"#rm_otp_enter_otp","hide":"#rm_noelement","reload":false,"msg":"'.__('Your IP has been banned by the Admin due to repeated failed login attempts.','registrationmagic-gold').'"}';exit;
        }
        ...
        if (null != $user_email) {
            ...
            if (isset($request->req['submission_id'])) {
                ...
                if ($submission->get_user_email() == $user_email) {
                    ...
                    $form_type = $form->get_form_type() == "1" ?__('Registration','registrationmagic-gold') : __('Non WP Account','registrationmagic-gold');
    ...
    public function user_list($model, $service, $request, $params){ 
       ...
       $view = $this->mv_handler->setView('user_list',true);
       return $view->read($data);
   }
   
   protected function get_inbox_data($user_email, $service, $request, $params)
    {
        $data = new stdClass;
