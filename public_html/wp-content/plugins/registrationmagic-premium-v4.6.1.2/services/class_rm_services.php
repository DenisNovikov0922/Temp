...
class RM_Services {
    ...
    public function get_editable_fields($form_id) {
        ...
        if ((int) $form_id) {
            $db_fields = $wpdb->get_results($wpdb->prepare("SELECT * FROM `$table_name` WHERE `form_id` = %d AND `field_is_editable` = 1 AND `field_type` != 'Price' ORDER BY `page_no` ASC, `field_order` ASC",$form_id));
    ...
    private function get_headerdata() {
        ...
        $headerdata['title'] = __('Submission','registrationmagic-gold');
