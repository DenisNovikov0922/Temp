...
if(strtolower($data->model->field_type)=="username" || strtolower($data->selected_field)=='username'){
    $form->addElement(new Element_Textarea("<b>".__('Username exists error','registrationmagic-gold')."</b>", "user_exists_error", array("class" => "", "value" =>$data->model->field_options->user_exists_error , "longDesc" => sprintf(__('Contents of the error message user sees when trying to register with existing or blacklisted/ reserved username. You can manage blacklisted/ reserved usernames in Global Settings → Security. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-gold'),'https://registrationmagic.com/knowledgebase/username-field-settings/#htexisterror'))));
    $form->addElement(new Element_Checkbox("<b>".__('Allowed Characters','registrationmagic-gold')."</b>", "username_characters", array('alphabets' => __('Alphabets','registrationmagic-gold'),'numbers'=>__('Numbers','registrationmagic-gold'),'underscores'=>__('Underscores','registrationmagic-gold'),'periods'=>__('Periods','registrationmagic-gold')), array("class" => "rm_field_multiline rm_input_type", "value" => $data->model->field_options->username_characters, "longDesc"=>sprintf(__('Select the type of characters accepted for usernames. If none selected, All the characters would be allowed. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-gold'),'https://registrationmagic.com/knowledgebase/username-field-settings/#htallowedchars'))));
    $form->addElement(new Element_Textarea("<b>".__('Invalid username format error','registrationmagic-gold')."</b>", "invalid_username_format", array("class" => "", "value" => $data->model->field_options->invalid_username_format, "longDesc" => sprintf(__('Contents of the error message user sees when trying to enter username with characters not allowed. Use code {{allowed_characters}} to insert allowed characters dynamically. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-gold'),'https://registrationmagic.com/knowledgebase/username-field-settings/#htinvalidformat'))));
}
if(strtolower($data->model->field_type)=="userpassword" || strtolower($data->selected_field)=='userpassword'){
    $form->addElement(new Element_Checkbox("<b>".__('Display password confirmation field','registrationmagic-gold')."</b>", "en_confirm_pwd", array('1'=>''), array("class" => "rm_field_multiline rm_input_type en_confirm_pwd", "value" => $data->model->field_options->en_confirm_pwd, "longDesc"=>sprintf(__('Display a password confirmation field below the password field. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-gold'),'https://registrationmagic.com/knowledgebase/password-field-settings/#htdisplayconfirmation'))));
        $form->addElement(new Element_HTML('<div class="childfieldsrow" id="confirm_pwd_options">'));
             $form->addElement(new Element_Textarea("<b>".__('Password match error','registrationmagic-gold')."</b>", "pass_mismatch_err", array("class" => "", "value" =>$data->model->field_options->pass_mismatch_err , "longDesc" => sprintf(__('Contents of the error message user sees when the password entered in password confirmation field does not matches. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-gold'),'https://registrationmagic.com/knowledgebase/password-field-settings/#htmatcherror'))));
        $form->addElement(new Element_HTML('</div>'));
    ...
    if($enable=='yes'){
       $form->addElement(new Element_HTML('<div class="rm-disable">'));  
        ...
        $form->addElement(new Element_Checkbox("<b>".__('Display password strength meter','registrationmagic-gold')."</b>", "en_pass_strength", array('1'=>''), array("class" => "rm_field_multiline rm_input_type en_pass_strength ", "value" => $data->model->field_options->en_pass_strength, "longDesc"=>sprintf(__('Displays a live password strength meter to provide feedback to the user about the strength of the password they are entering. For advance password rules, checkout Global Settings → Security. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-gold'),'https://registrationmagic.com/knowledgebase/password-field-settings/#htstrengthmeter'))));   
        $form->addElement(new Element_HTML("<div class='childfieldsrow' id='pwd_strength_options'>"));
            $form->addElement(new Element_Textbox("<b>".__('Short','registrationmagic-gold')."</b>", "pwd_short_msg", array("class" => "rm_static_field", "value" => $data->model->field_options->pwd_short_msg, "longDesc"=>'')));
            $form->addElement(new Element_Textbox("<b>".__('Weak','registrationmagic-gold')."</b>", "pwd_weak_msg", array("class" => "rm_static_field", "value" => $data->model->field_options->pwd_weak_msg, "longDesc"=>'')));
            $form->addElement(new Element_Textbox("<b>".__('Medium','registrationmagic-gold')."</b>", "pwd_medium_msg", array("class" => "rm_static_field", "value" => $data->model->field_options->pwd_medium_msg, "longDesc"=>'')));
            $form->addElement(new Element_Textbox("<b>".__('Strong','registrationmagic-gold')."</b>", "pwd_strong_msg", array("class" => "rm_static_field", "value" => $data->model->field_options->pwd_strong_msg, "longDesc"=>'')));
        $form->addElement(new Element_HTML('</div>'));
    if($enable=='yes'){
       $form->addElement(new Element_HTML('</div>'));  
    }    
...
//Button Area
$form->addElement(new Element_HTMLL('&#8592; &nbsp; '.__('Cancel','registrationmagic-gold'), '?page=rm_field_manage&rm_form_id='.$data->form_id, array('class' => 'cancel')));
