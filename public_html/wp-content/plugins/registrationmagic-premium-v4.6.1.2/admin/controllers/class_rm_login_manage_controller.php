...
class RM_Login_Manage_Controller {
    ...
    public function sett_redirections($model, $service, $request, $params){
        ...
        if($this->mv_handler->validateForm("add-login-redirection")) {
            if($request->req['redirection_type']=='role_based' && empty($request->req['role_based_login_redirection'])){
                RM_PFBC_Form::setError('add-login-redirection',__('You must select the role to implement the redirections.','registrationmagic-gold'));
            }else{
                $params= array();
    ...
    public function val_sec($model, $service, $request, $params){
        ...
        if($this->mv_handler->validateForm("add-login-validation")) {
            ...
            $params['allowed_attempts_before_ban']= absint($request->req['allowed_attempts_before_ban']);
            $params['allowed_duration_before_ban']= absint($request->req['allowed_duration_before_ban']);
            $params['ban_type']= sanitize_text_field($request->req['ban_type']);
            $params['ban_duration']= absint($request->req['ban_duration']);
            $params['ban_error_msg']= $request->req['ban_error_msg'];
    ...
    public function recovery($model, $service, $request, $params){
        ...
        if($this->mv_handler->validateForm("login-recovery")) {
            ...
            if(!empty($params['en_pwd_recovery']) && empty($params['recovery_link_text'])){
                RM_PFBC_Form::setError('login-recovery',__('You must define text for the link.','registrationmagic-gold'));
            }
            else
            {
    ...
    public function two_factor_auth($model, $service, $request, $params){
        $data= new stdClass();
        if($this->mv_handler->validateForm("login-auth-options")) {
            $params= array();
        ...
        $user_service= new RM_User_Services();
        $data->roles= $user_service->get_user_roles();
        $view = $this->mv_handler->setView("login_two_factor_auth");
        $view->render($data);
    }
