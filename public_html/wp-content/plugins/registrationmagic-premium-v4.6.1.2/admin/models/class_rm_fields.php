...
class RM_Fields extends RM_Base_Model
{
    ...
    private function validate_form_id()
    {
        ...
        {
            $this->errors['FORM_ID'] = __("No Form ID defined",'registrationmagic-gold');
        }
        if (!is_int($this->form_id))
        {
            $this->errors['FORM_ID'] = __("Not a valid form ID",'registrationmagic-gold');
        }
    }
    ...
    private function validate_label()
    {
        ...
        {
            $this->errors['LABEL'] = __("Label can not be empty.",'registrationmagic-gold');
        }
        if (!is_string($this->field_label))
        {
            $this->errors['LABEL'] = __("Label must be a string.",'registrationmagic-gold');
        }
        if (preg_match('/[^a-zA-Z0-9_\-\.]/', $this->field_label))
        {
            $this->errors['LABEL'] = __("Label can not contain special characters.",'registrationmagic-gold');
        }
    }
    ...
    private function validate_type()
    {
        ...
        {
            $this->errors['TYPE'] = __("Field can not be empty.",'registrationmagic-gold');
        }
    ...
    private function validate_order()
    {
        ...
        {
            $this->errors['ORDER'] = __("Field order can not be empty.",'registrationmagic-gold');
        }
        if (is_int($this->field_order))
        {
            $this->errors['ORDER'] = __("Invalid Order",'registrationmagic-gold');
        }
    }
