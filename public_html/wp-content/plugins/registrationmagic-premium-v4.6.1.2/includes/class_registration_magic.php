...
class Registration_Magic
{
    ...
    /**
     * Hide/Show Toolbar before overall plugin loads
     * @access   private
     */
    ...
    private function define_admin_hooks()
    {
        ...
        $this->loader->add_action('wp_ajax_review_banner_handler', 'RM_Utilities', 'disable_review_banner');
        ...
        $this->loader->add_action('rm_pre_admin_template_render', $rm_admin, 'add_version_header');          $this->loader->add_action('wp_ajax_rm_one_time_action_update', 'RM_Utilities', 'update_action_state_ajax');
    ...
    public function define_global_hooks()
    {
        ...
        //$this->loader->add_action('wp_login_failed', $this, 'login_failed');
    ...
    public function prevent_deactivated_logins($user_login, $user = null)
    {
        ...
        if ($is_disabled == 1 && empty($prov_acc_act))
        {
            ...
            $goto = add_query_arg(array('is_disabled'=>'1','rm_user'=>$user->ID), $goto);
    ...
    {  
        ...
        $sub_page_url = '<a href="'.$sub_page_url.'">'.__('Click Here','registrationmagic-gold').'</a>';
    ...
    {
        //static $plugin;
        global $rmbasic, $rmsilver, $rmgoldi2;
        
        $base_plugin_dir = plugin_dir_path(plugin_dir_path(plugin_dir_path(__FILE__)));
        
        $plugin = plugin_dir_path(plugin_dir_path(__FILE__)) . "registration_magic.php"; 
        
        if ($plugin == $base_plugin_dir . $plugin_file)
        {
            $settings = array('settings' => '<a href="' . get_admin_url() . 'admin.php?page=rm_options_manage">'.__('Settings','registrationmagic-gold').'</a>');
            $site_link = array('support' => '<a href="' . get_admin_url() . 'admin.php?page=rm_support_forum">'.__('Support','registrationmagic-gold').'</a>');

            $actions = array_merge($site_link, $actions);
            $actions = array_merge($settings, $actions);
        }
        
        if ($rmgoldi2 == $plugin_file && isset($actions['activate']))
        {
            $localized_activate_text = strip_tags($actions['activate']);
            $actions['activate'] = '<a href="javascript:void(0)" onclick="alert(\''.RM_UI_Strings::get('NOTICE_GOLD_i2_ACTIVATION').'\')">'.$localized_activate_text.'</a>';
        }
        
        if ($rmsilver == $plugin_file && isset($actions['activate']))
        {
            $localized_activate_text = strip_tags($actions['activate']);
            $actions['activate'] = '<a href="javascript:void(0)" onclick="alert(\''.RM_UI_Strings::get('NOTICE_SILVER_ACTIVATION').'\')">'.$localized_activate_text.'</a>';
        }
        
        if ($rmbasic == $plugin_file && isset($actions['activate']))
        {
            $localized_activate_text = strip_tags($actions['activate']);
            $actions['activate'] = '<a href="javascript:void(0)" onclick="alert(\''.RM_UI_Strings::get('NOTICE_BASIC_ACTIVATION').'\')">'.$localized_activate_text.'</a>';
