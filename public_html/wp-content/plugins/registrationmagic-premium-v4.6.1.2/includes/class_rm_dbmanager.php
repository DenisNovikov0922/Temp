...
class RM_DBManager
{
   ...
   public static function get_all_form_attachments($form_id,$selection='value')
    ...
    public static function get_submissions($form_id=0,$filter,$selection="*",$sort_by = 'submission_id', $descending = true, $result_type='results', $paginated = true){
        if (!(int) $form_id)
            return false;
        global $wpdb;
        $wpdb->query('SET time_zone = "+00:00"');
        ...
        if($filters['custom_status_ind']!=null){
                ...
                foreach($filter_tags as $filter_tag):
                    switch(strtolower($filter_tag)){
                        ...
                        case strtolower(trim(RM_UI_Strings::get("LABEL_PENDING_OFFLINE_PAYMENTS"))):
                            ...
                            $pending_submission_ids= self::get_submissions_payment_status($form_id,"'pending','Pending','".__('Pending', 'registrationmagic-gold')."','".strtolower(__('Pending', 'registrationmagic-gold'))."'");
