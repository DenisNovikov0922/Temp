...
ob_start();
?>
<form id="wpsc_frm_add_agent" method="post" action="javascript:wpsc_set_add_new_agent_rule();">
  	<div class="form-group">
			<label for="wpsc_caa_title"><?php _e('Title','wpsc-caa');?></label>
			<p class="help-block"><?php _e('Title to show in rules list. It will be easier to know what this rule is for.','wpsc-caa');?></p>
			<input id="wpsc_caa_title" class="form-control" name="wpsc_caa_title" value="" required />
		</div>
		<div class="form-group">
	    <label for="wpsc_agent_name"><?php _e('Select Agents','wpsc-caa');?></label>
	    <p class="help-block"><?php _e('Search agents by their names..','wpsc-caa');?></p>
			<div id="assigned_agent">
			  <div class="form-group wpsc_display_assign_agent ">
				    <input id="wpsc_agent_name" class="form-control form-control wpsc_agent_name ui-autocomplete-input" name="assigned_agent"  type="text" autocomplete="off" placeholder="<?php _e('Search agent ...','wpsc-caa')?>" />
						<ui class="wpsp_filter_display_container"></ui>
				</div>
		  </div>
		</div>
		<div class="form-group">
			<label for=""><?php _e('Conditions','wpsc-caa');?></label>
			<p class="help-block"><?php _e('Set conditions to match when new ticket is created to apply this rule.','wpsc-caa');?></p>
			<div class="row">
		  	<ul id="wpsc_tf_condition_container" class="wpsp_filter_display_container"></ul>
		  </div>
			<?php $wpscfunction->load_conditions_ui('wpsc_add_agent_rule_conditions');?>
		</div>
		<input type="hidden" name="action" value="wpsc_set_add_new_agent_rule" />
</from>
