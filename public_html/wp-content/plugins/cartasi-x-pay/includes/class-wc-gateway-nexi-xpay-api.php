...
class WC_Gateway_XPay_Api
{
    ...
    public function set_credentials_oneclick($aliasOneClick, $chiaveSegretaOneClick)
    {
    ...
    public function refund($codiceTransazione, $importo)
    {
        ...
        $importo = round($importo, 2) * 100; // 5000 = 50,00 EURO (indicare la cifra in centesimi)
    ...
    public function recurring_payment($num_contratto, $scadenza_pan, $amount_to_charge, $newCodTrans, $note1 = null, $note2 = null, $note3 = null)
    {
        ...
        $importo = round($amount_to_charge,2) * 100; // 5000 = 50,00 EURO (indicare la cifra in centesimi)
        $divisa = "978"; // divisa 978 indica EUR
        $timeStamp = (time()) * 1000;
    ...
    public function order_detail($codiceTransazione, $recurring = false)
    {
        ...
        if ($this->response['mac'] == $MACrisposta) {
            ...
            if ($this->response['esito'] == 'OK') {
                ...
                $this->log_action('info', sprintf(__("Detail order %s operation was rejected. Error detail: %s", 'woocommerce-gateway-nexi-xpay'), $codiceTransazione, $this->response['errore']['messaggio']));
                return new WP_Error('broke', sprintf(__("Detail order %s operation was rejected. Error detail: %s", 'woocommerce-gateway-nexi-xpay'), $codiceTransazione, $this->response['errore']['messaggio']));
            }
        } else {
            $this->log_action('info', __('Error in the calculation of the return MAC parameter', 'woocommerce-gateway-nexi-xpay'));
            return new WP_Error('broke', __('Error in the calculation of the return MAC parameter', 'woocommerce-gateway-nexi-xpay'));
