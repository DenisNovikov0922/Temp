...
/*        add_action('woocommerce_receipt_' . $this->id , array($this, 'wc_xpay_page_invioform'));*/
   ...
   add_action('woocommerce_receipt_' . $this->id , array($this, 'wc_xpay_page_invioform'));
    ...
    public function wc_xpay_genera_form($order_id)
    {
        ...
        $submit_form = '<form action="' . $this->url_pagina_cassa . '" method="post" id="nexi_xpay_payment_form" enctype="application/www-x-form-urlencoded">
                                        ' . $nexi_xpay_inputs . '
    ...
    public function get_params_form($order_id)
    {
        ...
        $importo = (round($order->get_total(), 2) * 100);
        ...
        if ($this->settings['enabled3ds'] == 'yes' && get_called_class() != WC_Gateway_XPay_APM::class) {
    ...
    public function get_params_3ds2($order_id)
    {
        ...
        require_once('class-wc-gateway-nexi-xpay-iso3166.php');
        ...
        if ($order->get_billing_phone() != '' && preg_match('/^(\+)([0-9]{10,15})$/', $order->get_billing_phone())) {
    ...
    public function get_available_methods()
    {
        ...
        $xpayAPI = new WC_Gateway_XPay_Api();
        $settings = WC_Admin_Settings::get_option('woocommerce_xpay_settings');
        $xpayAPI->set_env($settings['cartasi_modalita_test'] == yes ? $this->url_test : $this->url_produzione);
        $xpayAPI->set_credentials($settings['cartasi_alias'], $settings['cartasi_mac']);
        $avaiable_methods = $xpayAPI->get_available_methods($woocommerce->version, WC_GATEWAY_XPAY_VERSION);
        if ($avaiable_methods) {
            update_option('xpay_available_methods', json_encode($xpayAPI->response['availableMethods']));
            update_option('xpay_logo_small', $xpayAPI->response['urlLogoNexiSmall']);
            update_option('xpay_logo_large', $xpayAPI->response['urlLogoNexiLarge']);
        } else {
