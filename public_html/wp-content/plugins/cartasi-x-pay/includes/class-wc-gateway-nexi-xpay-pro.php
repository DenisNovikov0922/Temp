...
class WC_Gateway_XPay_Pro extends WC_Gateway_XPay_Easy
{
    ...
    {
        parent::set_api();
        $this->API->set_credentials_oneclick($this->alias, $this->chiave_segreta);
    }
    ...
    public function get_params_form($order_id)
    {
        ...
        if ($_REQUEST['xpay-oc'] === "true" && !$this->order_contains_subscription($order_id)) {
            //Recupero ID cliente
...
/*            $params['tipo_servizio'] = "paga_multi";*/
/*            $params['tipo_richiesta'] = "PP"; //Primo Pagamento*/
/*            $params['gruppo'] = $this->gruppo_oneclick; //Da config*/
            if ($this->settings['abilita_modulo_oneclick'] == 'yes' && !$this->order_contains_subscription($order_id) && is_user_logged_in()) {
                $params['num_contratto'] = substr("OCXP_" . $costumer_id . "_" . WC_Admin_Settings::get_option('nexi_unique'), 0, 30); //Numero contratto con XPAY
                $params['codTrans'] = $this->get_cod_trans($order_id);
                $params['gruppo'] = '';
                $params['tipo_servizio'] = 'paga_1click';
                $params['mac'] = $this->get_mac_calculated_oc3d($params);
                unset($params['tipo_richiesta']);
            } else {
    ...
    protected function one_click_payment_process($order_id)
    {
        ...
        if ($this->API->recurring_payment($token->get_token(), $scadenza_pan, $order->get_total(), $cod_trans, 'woocommerce', $this->wpbo_get_woo_version_number_nexi(), $this->module_version) === true) {
            ...
            $aInfoPayment['importo'] = round($order->order_total, 2) * 100;
