...
class WC_Gateway_XPay extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->method_description .= '<br><p>' . __('A POST notification by the Nexi servers is sent to the following address, containing information on the outcome of the payment.', 'woocommerce-gateway-nexi-xpay') . '<br><b>' . home_url('?wc-api=WC_Gateway_XPay') . '</b><br>' . __('The notification is essential for the functioning of the plugin, it is therefore necessary that it is not blocked or filtered by the site infrastructure.', 'woocommerce-gateway-nexi-xpay') . '</p>';
        $this->id = 'xpay';
    ...
    protected function set_description()
    {
        ...
        $this->description = $img_list . __("Pay securely by credit and debit card through Nexi.", 'woocommerce-gateway-nexi-xpay') .
    ...
    protected function get_mac_calculated_oc3d($params)
    {
        return sha1('codTrans=' . $params['codTrans'] . 'divisa=' . $params['divisa'] . 'importo=' . $params['importo'] . 'gruppo=' . $params['gruppo'] . 'num_contratto='
        . $params['num_contratto']  . $this->chiave_segreta);
    }
        ...
        if (isset($_REQUEST['codTrans']) && isset($_REQUEST['esito']) && !$macError) {
            ...
            if ($order_id != '') {
                try {
                    ...
                    $order_total = (round($order->get_total(), 2) * 100);
                    ...
                    if ((abs($order_total - $nexi_order_total)) < 1) {
                        ...
                        if ($esito == 'OK') {
                            ...
                            $msg['class'] = 'error';
                            $msg['message'] = __("Thank you for shopping with us. However, the transaction has been declined.", 'woocommerce-gateway-nexi-xpay') . " - " . $_REQUEST['messaggio'];
                    ...
                    $msg['class'] = 'error';
                    $msg['message'] = __("Thank you for shopping with us. However, the transaction has been declined.", 'woocommerce-gateway-nexi-xpay') . " - " . $e->getMessage();
    ...
    public function save_payment_method_checkbox()
    {
        printf(
                ...
                <input checked hidden id="wc-%1$s-new-payment-method" name="wc-%1$s-new-payment-method" type="checkbox" value="true" style="width:auto;" />
            </p>',
            esc_attr($this->id)
        );
    }
