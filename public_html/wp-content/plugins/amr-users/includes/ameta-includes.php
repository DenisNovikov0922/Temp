...
function debug_this($what, $text='' ) {
	echo '<pre>'.$text.' '.print_r($what, true).'</pre>';
}
	
function amr_get_local_url_if_file_exists($partialurl, $args) {
	...
	// if image and size requested, check for matching file size
		...
		if (!file_exists($localfile)) { // if the original file dpes not exist
			debug_this($localfile,' local file');
				return false;	
		}
		$url = array();
...
function amru_new_DateTime_nonfatal ($datetimestring, $tzobj) { // date time create with exception trap to avoid fatal errors
		try {	
			...
			sprintf(__('Unable to create DateTime object from this string: "%s" ','amr-users'),$datetimestring).' with timezone '.print_r($tzobj)
