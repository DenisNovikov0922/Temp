    ...
    class IC_Commerce_Mutlisite_Reporting_Functions{
		...
		*/
		function get_order_status(){
			$order_status = "wc-".implode(",wc-", apply_filters( 'woocommerce_reports_order_statuses',array('completed','processing','on-hold','on-refunded')));
		...
		*/
		...
		/*
			...
			*
		*/
		function get_blogs_of_user($user_id = 0, $all = false){												$bg =  array();						$sites = get_sites();			foreach ($sites as $site ) {				$x = (object) ["userblog_id" => $site->blog_id,"blogname" => $site->blogname, "domain" => $site->domain,"path" => $site->path,"site_id" => $site->site_id, "siteurl" => $site->siteurl, "archived" => $site->archived, "mature" => $site->mature, "spam" => $site->spam,"deleted" => $site->deleted];$bg[] = $x;			}			return $bg;												/*
