	...
	function check_url_rules( $type, $rules, $uri ) {
		...
		foreach ( $rules as $_key => $_codes ) {
			...
			switch ( $type ) {
				case 'contains':
				case 'not-contains':
					if ( ! empty( $_key ) && strpos( $uri, $_key ) !== false ) {
						$accept = true;
					}
					if ( ! empty( $_key_relative ) && strpos( $uri, $_key_relative ) !== false ) {
						$accept = true;
					}
					break;
				case 'equal-to':
				case 'not-equal-to':
					if ( $uri == $_key || $uri == $_key_relative ) {
						$accept = true;
					}
					break;
				case 'begins-with':
					if ( ! empty( $_key ) && strpos( $uri, $_key ) === 0 ) {
						$accept = true;
					}
					if ( ! empty( $_key_relative ) && strpos( $uri, $_key_relative ) === 0 ) {
						$accept = true;
					}
					break;
				case 'ends-by':
					if ( ! empty( $_key ) && strpos( strrev( $uri ), strrev( $_key ) ) === 0 ) {
						$accept = true;
					}
					if ( ! empty( $_key_relative ) && strpos( strrev( $uri ), strrev( $_key_relative ) ) === 0 ) {
						$accept = true;
					}
					break;
			}
