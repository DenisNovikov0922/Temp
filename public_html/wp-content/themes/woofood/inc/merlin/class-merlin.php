...
class Merlin {
	...
	protected function license() {
		...
		$no_license = $strings['no-license-link'];
		?>
	...
	protected function content() {
				...
				<div class="merlin__select-control-wrapper">
					...
					<select class="merlin__select-control js-merlin-demo-import-select">
						<?php foreach ( $this->import_files as $index => $import_file ) : ?>
							<option value="<?php echo esc_attr( $index ); ?>" preview-img="<?php echo $import_file['import_preview_image_url'];?>"><?php echo esc_html( $import_file['import_file_name'] ); ?></option>
						<?php endforeach; ?>
					</select>
								<h1>Preview Screenshot</h1>

		<img class="preview-img" width="100%" height="auto" src="<?php echo $this->import_files[0]['import_preview_image_url']; ?>"/>
		...
		<script>
		jQuery('.js-merlin-demo-import-select').on('change', function()
                                           {  var url =   jQuery('option:selected', this).attr('preview-img');    jQuery(".preview-img").attr("src", url);
   });
		</script>
	...
	protected function edd_activate_license( $license ) {
		...
		// Text strings.
		$success_message = esc_html__('WooFood Theme and Plugin has been succesfully activated', 'woofood');
		$already_active_message = esc_html__('Theme is already active on other url', 'woofood');
		$invalid_license_message = esc_html__('License Key is Invalid', 'woofood');

		$message = sprintf( esc_html( $success_message ), $theme );
		$message_a = sprintf( esc_html( $already_active_message ), $theme );
		$message_i = sprintf( esc_html( $invalid_license_message ), $theme );

		  $ch = curl_init();
     
    // Set cURL options
    curl_setopt($ch, CURLOPT_URL, "https://www.wpslash.com/licensing/envato-license-check.php?purchase_code=".$license."&domain=".$site_url );
    ...
    // Close Channel
    curl_close($ch);	
    $woofood_plugin_options = get_option( 'woofood_options' );
    if($woofood_plugin_options)
    {

    }
    else
    {
    	$woofood_plugin_options =  array();
    }
        ...
        $woofood_plugin_options['woofood_license_number'] = $license ;
        update_option('woofood_options', $woofood_plugin_options );
		...
		$success = false;
		$message = sprintf( esc_html( $already_active_message ), $theme );

        $woofood_plugin_options ['woofood_license_number'] = "Already Active on other Domain";
		return compact( 'success', 'message' );
		...
		$success = false;
		$message = sprintf( esc_html( $invalid_license_message ), $theme );

        $woofood_plugin_options['woofood_license_number'] = "Invalid License";
		return compact( 'success', 'message' );
	...
	public function after_content_import_setup() {
		// Set static homepage.
		$homepage = get_page_by_title( 'FrontPage from Template' );
